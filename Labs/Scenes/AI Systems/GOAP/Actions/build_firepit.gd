...
## firepit scene in ResourcesManager's memory (might change to StructureManager), and
...
func perform(agent: HumanAiGoap, _delta: float) -> bool:
	if ResourcesManager.valid_spot:
		# FIXME - The second parameter is disgusting, look at that shit! Fuck, it's below me now!
		ResourcesManager.place_structure(spot, agent.controller.get_parent().get_parent().get_parent())
		try = false
		...
		ResourcesManager.move_structure_area(spot)
	else:
		try = true
		ResourcesManager.prepare_structure("firepit", agent.controller.get_parent().get_parent().get_parent())
		if not area:
