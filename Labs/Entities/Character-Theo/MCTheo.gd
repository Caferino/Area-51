class_name MCOldTheo extends CharacterBody3D
...
#func handle_blocks(delta):
	...
	#var ray_cast_from_mouse = get_world_2d().direct_space_state.intersect_ray(get_viewport().get_camera().project_ray_origin(mouse_position), get_viewport().get_camera().project_ray_normal(mouse_position))
#
	#if ray_cast_from_mouse:
		#BlockHighlight.Visible = true
#
		#var block_position = ray_cast_from_mouse.position
		#var int_block_position = Vector3(floor(block_position.x), floor(block_position.y), floor(block_position.z))
		#BlockHighlight.GlobalPosition = int_block_position + Vector3(0.5, 0.5, 0.5)
#
