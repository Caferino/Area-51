...
signal content_changed()  ## Emitted when an item is added, removed or changed
signal size_changed()  ## Emitted if the size is changed (not used for now)
...
## Set its own and the slots' groups
...
## Try to place the stackable items in the array
## If they can be placed, and the stack is empty, they're removed
...
func set_upgradable_items():
	for s in slots:
		if s.item and s.item.type == GameEnums.ITEM_TYPE.EQUIPMENT and s.item.rarity < GameEnums.RARITY.RARE:
...
## When an item from a slot changes, the content has changed
