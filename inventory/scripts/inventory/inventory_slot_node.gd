class_name InventorySlotNode extends TextureRect
...
func set_slot(value):
	...
	if slot and slot.item:
		var item_node = AlcarodianResourceManager.get_item_node(slot.item)
...
## When the slot is shown or hidden, cause a mouse_exited signal so the item info hides
func _on_item_container_visibility_changed():
	mouse_exited.emit()


## When the item changes, update the visual
func _on_item_changed():
	for c in %item_container.get_children():
		c.queue_free()
	
	if slot.item:
		%item_container.add_child(AlcarodianResourceManager.get_item_node(slot.item))
