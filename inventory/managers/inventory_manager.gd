...
## Remove the quantity of item_id contained in the inventory group
...
## Return the Inventory from its name
...
## Hide the item's info
func _on_mouse_exited_slot():
	%item_info.hide()


## Detect and process the different clicks on an inventory slot node
...
## Ex: Enable the craft button if the crafting inventories have the requires items
